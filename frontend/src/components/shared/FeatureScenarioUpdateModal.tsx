...
export function FeatureScenarioUpdateModal({ open, onClose, projectId, feature }: Props) {
  ...
  const analyzeMutation = useMutation({
    mutationFn: () => apiClient.post(`/projects/${projectId}/ai/update-scenarios-for-feature`, {
    ...
    onSuccess: async (data) => {
      if (Array.isArray(data)) {
        ...
        if (actionable.length === 0) {
          await projectApi.clearOutdatedScenariosByFeature(projectId, feature.id)
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const item of selected) {
        if (item.action === 'update' && item._existingScenarioId) {
          await testApi.updateScenario(projectId, item._existingScenarioId, { title: item.title, description: item.description })
        } else if (item.action !== 'skip') {
          await testApi.createScenario(projectId, { title: item.title, description: item.description, featureId: feature.id, type: 'integration', status: 'draft' } as any)
        }
      }
    },
    onSuccess: async () => {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="AI 테스트 시나리오 업데이트" className="max-w-lg">
      <div className="space-y-3">
        {step === 'confirm' && (
          <div className="space-y-3">
            <div className="px-3 py-2 bg-gray-50 rounded-md border">
              <p className="text-[11px] text-gray-400">대상 기능</p>
              <p className="text-xs font-medium mt-0.5">
                <span className="text-[#5E6AD2] font-mono">{feature.code}</span>
                <span className="text-gray-700 ml-1.5">{feature.title}</span>
              </p>
            </div>
            <p className="text-xs text-gray-600 leading-relaxed">
              AI가 이전 확정 이력과 현재 기능을 비교하여 테스트 시나리오를 업데이트합니다.
            </p>
              ...
              {results.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.action === 'update' && item._existingCode && <p className="text-[10px] text-amber-600 mt-0.5">← {item._existingCode} 수정</p>}
