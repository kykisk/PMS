...
export function MultiTaskGenerateModal({ open, onClose, projectId }: Props) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="다중 AI Task 생성" className="max-w-2xl">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-1 pt-3">
              ...
              <div className="flex items-center justify-between">
                ...
                {selectedIds.size >= MAX_SELECT && (
                  <span className="text-[10px] text-red-500 font-medium">최대 {MAX_SELECT}개까지 선택 가능</span>
                )}
              </div>
              ...
              <p className="text-[10px] text-gray-400">✓ 확정된 기능 중 Task가 없는 항목만 표시</p>
              <div className="border rounded-md divide-y">
                ...
                {filtered.length === 0 && (
                  <div className="py-6 text-center text-xs text-gray-400">확정된 기능이 없거나 모든 기능에 Task가 이미 생성되었습니다</div>
                )}
              </div>
            </div>
