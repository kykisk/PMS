...
interface Props {
  ...
  onClose: () => void
  projectId: string
  feature: { id: string; code: string; title: string; status: string }
}

export function FeatureTaskUpdateModal({ open, onClose, projectId, feature }: Props) {
  ...
  const analyzeMutation = useMutation({
    mutationFn: () => apiClient.post(`/projects/${projectId}/ai/update-tasks-for-feature`, {
    ...
    onSuccess: async (data) => {
      if (Array.isArray(data)) {
        ...
        if (actionable.length === 0) {
          await projectApi.clearOutdatedByFeature(projectId, feature.id)
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const item of selected) {
        if (item.action === 'update' && item._existingTaskId) {
          await taskApi.update(projectId, item._existingTaskId, { title: item.title, description: item.description })
        } else if (item.action !== 'skip') {
          await taskApi.create(projectId, { title: item.title, description: item.description, featureId: feature.id, status: 'pending' } as any)
        }
      }
  ...
  return (
    <Modal open={open} onClose={handleClose} title="AI Task 업데이트" className="max-w-lg">
        ...
        {step === 'confirm' && (
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              AI가 이전 확정 이력과 현재 기능을 비교하여 Task를 업데이트합니다.
            </p>
