...
export class TraceabilityService {
  ...
  async getMatrix(projectId: string) {
    const [requirements, features, tasks, scenarios] = await Promise.all([
      ...
      this.prisma.testScenario.findMany({
        where: { projectId },
        include: { testCases: { select: { id: true, result: true, status: true } } },
      }),
    ]);
      ...
      return {
        ...
        features: reqFeatures.map(f => {
          ...
          return {
            ...
            testScenarios: featureScenarios.map(s => ({
              id: s.id, code: s.code, title: s.title,
              result: this.aggregateResult(s.testCases),
            })),
          };
        }),
        directScenarios: reqScenarios.map(s => ({
          id: s.id, code: s.code, title: s.title,
          result: this.aggregateResult(s.testCases),
        })),
      };
  ...
  async getCoverage(projectId: string) {
    const [reqCount, featureCount, taskCount, scenarioCount] = await Promise.all([
    ...
    const reqWithFeature = await this.prisma.requirement.count({
      where: { projectId, features: { some: {} } },
    });
    const featureWithTask = await this.prisma.feature.count({
      where: { projectId, tasks: { some: {} } },
    });
    const featureWithTest = await this.prisma.feature.count({
      where: { projectId, testScenarios: { some: {} } },
    });
    const passedCases = await this.prisma.testCase.count({
      where: { scenario: { projectId }, result: 'pass' },
    });
